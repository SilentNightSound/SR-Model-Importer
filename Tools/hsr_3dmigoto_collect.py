...
# Big thanks to LeoTorreZ#6973 and NicoMico#7692 for testing and development

# Currently only tested for star rail, but may work for other mihoyo games if they are similar enough
...
def main():
    ...
    parser.add_argument("-ib", nargs="+", type=str, help="Main IBs character is drawn with")
    ...
    parser.add_argument("-vs", type=str, default="e8425f64cfb887cd", help="Root VS for character model")
    ...
    if args.ib:
        print("Using IB instead of VB to find parts")
        args.vb = find_draw_vb(frame_dump_folder, args.ib)
    elif args.vb:
        input("WARNING: Star rail has 3 different VBs that cause the character to vanish, but only one of them is correct. You can either find the correct vb using trial "
              "and error, or by looking at the frame dump. Otherwise, I recommend using -ib and passing the ib hash instead. Press enter to continue")
    ...
    for i in range(len(model_data)):
        ...
        # Some objects do not have a separate vb1, and all the data is contained in vb0
            ...
            if args.no_blend or (not args.force_object and not point_vb_candidates):
                ...
                blend, blend_format = collect_buffer_data(frame_dump_folder, blend_vb, ("BLENDWEIGHTS:", "BLENDINDICES:"))
...
def collect_pointlist_candidates(frame_dump_folder, root_vs_hash="653c63ba4a73ca8b"):
    ...
    for filename in frame_dump_files:
        ...
        if root_vs_hash in filename and "-vb2=" in filename and os.path.splitext(filename)[1] == ".txt":
...
def find_draw_vb(frame_dump_folder, ib_hashes):
    frame_dump_files = os.listdir(frame_dump_folder)
    vb_hashes = []
    for ib_hash in ib_hashes:
        for name in frame_dump_files:
            if ib_hash in name:
                draw_id = name.split("-")[0]
                break
        vb_hash = [x.split("=")[1].split("-")[0] for x in frame_dump_files if draw_id in x and "vb0=" in x][0]
        vb_hashes.append(vb_hash)
    return vb_hashes
...
# In star rail, there are 3 vb hashes the user might pass in and only 1 is the draw vb
# This confirms that all the passed vbs are the draw vbs, and if not attempts to identify it
def check_draw_vb(frame_dump_files, draw_vb_hashes):
    print("Double checking vbs are draw vbs")
    for vb_hash in draw_vb_hashes:
        print(f"Checking {vb_hash}")

        discovered = {vb_hash}

        # First, we collect all draw IDs the hash shows up in
        draw_ids = set([x.split("-")[0] for x in frame_dump_files if vb_hash in x])

        # Then, we collect all vb hashes that show up in those draw IDs
        for draw_id in draw_ids:
            discovered.update(set([x.split("=")[1].split("-")[0] for x in frame_dump_files if f"{draw_id}-" in x and "vb" in x]))

        # Normally this would be enough, but we need one more step in the case where we passed in the blend hash
        # Wwe repeat the above for all the new vb hashes discovered
        print(discovered)
        for hash in discovered:
            draw_ids = set([x.split("-")[0] for x in frame_dump_files if hash in x])
            for draw_id in draw_ids:
                discovered.update(set([x.split("=")[1].split("-")[0] for x in frame_dump_files if f"{draw_id}-" in x and "vb" in x]))
