...
def export_3dmigoto_genshin(operator, context, object_name, vb_path, ib_path, fmt_path, use_foldername, ignore_hidden, only_selected, no_ramps, delete_intermediate, credit, Outline_Properties, star_rail=False):
    ...
    generate_mod_folder(os.path.dirname(vb_path), object_name, no_ramps, delete_intermediate, credit, star_rail)


def generate_mod_folder(path, character_name, no_ramps, delete_intermediate, credit, star_rail):
...
class Export3DMigotoStarRail(bpy.types.Operator, ExportHelper):
    """Export a mesh for re-injection into a game with 3DMigoto"""
    bl_idname = "export_mesh_genshin.starrail"
    bl_label = "Export Star Rail mod folder"
    bl_options = {'PRESET', 'UNDO'}
    ...
    def execute(self, context):
        try:
            ...
            # FIXME: ExportHelper will check for overwriting vb_path, but not ib_path
            Outline_Properties = (
            self.outline_optimization, self.toggle_rounding_outline, self.decimal_rounding_outline, self.angle_weighted,
            self.overlapping_faces, self.detect_edges, self.calculate_all_faces, self.nearest_edge_distance)
            export_3dmigoto_genshin(self, context, object_name, vb_path, ib_path, fmt_path, self.use_foldername,
                                    self.ignore_hidden, self.only_selected, self.no_ramps, self.delete_intermediate,
                                    self.credit, Outline_Properties, star_rail=True)
        except Fatal as e:
            self.report({'ERROR'}, str(e))
        return {'FINISHED'}
...
def menu_func_export_starrail(self, context):
    self.layout.operator(Export3DMigotoStarRail.bl_idname, text="Exports Star Rail Mod Folder")
...
def register():
    ...
    export_menu.append(menu_func_export_starrail)
...
def unregister():
    ...
    export_menu.remove(menu_func_export_starrail)
